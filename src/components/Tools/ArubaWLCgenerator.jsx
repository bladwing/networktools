  ...
  const handleSubmit = (event) => {
    ...
    const newCommands = lines
      ...
      .map((line) => {
        ...
        return `whitelist-db cpsec ${commandAction} mac-address ${formattedMac}
ap-name ${apName} ap-group ${selectedGroup} ${
          commandType === "add" ? "" : "state approved-ready-for-cert"
        } 
show audit-trail 1`;
      })
      ...
      <div className="row gx-5">
        <div className="col-md-6">
          ...
          {commands.length > 0 && (
            <div className="mt-3">
              <p>Commands:</p>
              <pre style={{textAlign: "left"}}>{commands.join("\n\n")}</pre>
            </div>
          )}
        </div>
        <div className="col-md-6">
          <p>
            Copy from Excel (port map) two columns: AP name and MAC:
          </p>
          <img
            src="/img/examplewcrename.jpg"
            alt="Example"
            className="img-fluid"
          />
        </div>
